...
> Stack: Next.js, PostgreSQL (Neon), Redis (Upstash), Vitest, Playwright
...
| Integration | Vitest + test DB (Neon branch) | API endpoints, DB queries, middleware, auth | 80%+ for API routes | Every PR |
...
### 3.3 Checkout (Experiencia de Compra)
...
describe('calculateCartTotal', () => {
  it('applies 5% PIX discount to subtotal after coupon', () => {
    const result = calculateCartTotal({
      items: [
        { sku: 'SKU-001', price: 199.90, quantity: 2 },
      ],
      coupon: { type: 'percentage', value: 0.10 },
      paymentMethod: 'pix',
    });
    // Subtotal: 399.80
  ...
  it('does not apply PIX discount when payment method is credit card', () => {
    ...
    expect(result.total).toBeCloseTo(199.90, 2);
    expect(result.installmentValue).toBeCloseTo(66.63, 2);
  });
});
```

### 3.4 PCP (Planejamento e Controle de Producao)
...
describe('calculateCommission', () => {
  ...
  it('adjusts commission when exchange happens within 7-day window', () => {
    const commission = calculateCommission({
      ...
      discount: 0,
      creatorTier: 'silver',
      exchangeAmount: 100.00,  // partial exchange
...
export function createContact(overrides: Partial<Contact> = {}): Contact {
  return {
    ...
    address: {
      ...
      state: faker.helpers.arrayElement(['SP', 'RJ', 'MG', 'RS', 'PR', 'SC', 'BA']),
    },
    rfmScore: { recency: 3, frequency: 3, monetary: 3, segment: 'potential_loyalists' },
...
export const handlers = [
  ...
  http.post('https://api.melhorenvio.com.br/api/v2/me/shipment/generate', () => {
    return HttpResponse.json({
      id: 'label-uuid',
  ...
  http.post('https://api.z-api.io/instances/:instance/token/:token/send-text', () => {
    return HttpResponse.json({ zapiMessageId: 'msg-uuid', messageId: 'wamid.xxx' });
  }),
...
### 7.3 Redis: real test instance

Redis (Upstash) is tested against a real test instance because:

- Redis operations are fast enough not to slow down tests
- Mocking Redis hides bugs in key naming, TTL logic, and data structure choices
- Test instance is separate from production (different Upstash database)
...
export const testRedis = new Redis({
  url: process.env.TEST_UPSTASH_REDIS_URL!,
  token: process.env.TEST_UPSTASH_REDIS_TOKEN!,
});

// Flush test Redis before each suite
beforeAll(async () => {
  await testRedis.flushdb();
});
```

### 7.4 Time: fake timers for cron-dependent logic
...
it('sends WhatsApp review request after order delivery', async () => {
  ...
  server.use(
    http.post('https://api.z-api.io/instances/:instance/token/:token/send-text', async ({ request }) => {
...
pnpm vitest run src/modules/erp
pnpm vitest run src/modules/creators/commission

# Run with coverage report
pnpm vitest run --coverage

# Run E2E tests (requires running dev server or staging URL)
pnpm exec playwright test

# Run single E2E test
pnpm exec playwright test test/e2e/checkout.spec.ts

# Run E2E with headed browser (visible)
pnpm exec playwright test --headed

# Debug E2E test (step through)
...
- **INFRA.md** — Neon branching, Upstash configuration, CI/CD setup
