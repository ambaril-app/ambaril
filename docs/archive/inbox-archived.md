...
**Out of scope:** This module does NOT own message delivery. WhatsApp messages are sent/received via the Mensageria module; email messages are sent/received via Resend integration. Inbox is the **interface layer** — WhatsApp Engine and Resend are the **transport layers**. This module does NOT own the exchange workflow — it triggers exchange creation in the Trocas module and displays linked exchange status. This module does NOT own CRM data — it reads from `crm.contacts` for the context panel.
...
| 3. Cutover       | Route all inbound WhatsApp messages through the new Inbox. Slimgust stops using WhatsApp Business directly. | Week 4    | Medium — requires WhatsApp Business number migration to Meta Cloud API (coordinated with Mensageria module rollout) |
...
_This module spec is the source of truth for Inbox de Atendimento implementation. All development, review, and QA should reference this document. Changes require review from Marcus (admin) or Slimgust (support)._
