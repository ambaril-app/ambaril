...
> **STATUS: READ-ONLY** — Yever continua processando vendas e checkout. Ambaril lê dados via API Yever para Dashboard/Analytics.
> Este spec é referência para implementação futura de checkout próprio.
> Módulo Checkout no roadmap atual = importação de dados Yever (read-only), não processamento ativo.
...
| OQ-9 | Future integration: CRO tools (heatmap, scroll tracking, session replay) via Microsoft Clarity (free) or Hotjar | Caio / Marcus | Open   | Provides click maps and conversion funnel visualization for checkout optimization. Microsoft Clarity is free and privacy-compliant; Hotjar offers more advanced features at cost. Enables data-driven checkout UX improvements: identify where users hesitate, which form fields cause friction, and where scroll depth drops. Requires script injection on public checkout pages and LGPD consent for session recording. Ref: Pandora96 uses CRO tooling to optimize conversion at every step. |

---

### Waitlist + VIP Whitelist para Drops

Pre-registration (email + WhatsApp). First 24h of drop = VIP-only (login required to purchase). After 24h = public. Waitlist data feeds PLM demand planning: "1,200 people want the black jacket" calibrates production. Launch notifications via Mensageria. VIP whitelist management in CRM.

### Recomendação de Tamanho

Customer enters height, weight, fit preference (tight/regular/oversized) → system suggests size. Client-side matching (JavaScript, no API call — instant rendering). Size table per SKU in database + modal/drawer in checkout. Exchange data from Trocas module refines model over time. Impact: reduces size exchanges = lower logistics cost + better NPS.

### Reviews & Ratings

1-5 star product reviews with photos and text. Auto-request via WA/email after delivery (integrates with CRM automation). Moderation: auto-approve by threshold, flag profanity/URLs, alert on low ratings. Public API for product page (Shopify embed). Verified purchase badge. Review photos → DAM as UGC.
...
_This module spec is the source of truth for Checkout implementation. All development, review, and QA should reference this document. Changes require review from Marcus (admin) or Caio (pm)._
