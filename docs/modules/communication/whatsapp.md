# WhatsApp Engine — Module Spec

> **Module:** WhatsApp Engine
> **Schema:** `whatsapp`
> **Route prefix:** `/api/v1/whatsapp`
> **Admin UI route group:** `(admin)/whatsapp/*`
> **Webhook route:** `/api/v1/webhooks/meta-whatsapp`
> **Version:** 1.0
> **Date:** March 2026
> **Status:** Approved
> **Phase:** 1A (Semanas 1-12). Originalmente Phase 2, movido para 1A com base na entrevista com Caio — todas as automacoes ativas sao WhatsApp-only.
> **Replaces:** None (new capability; previously manual WhatsApp Business)
> **References:** [DATABASE.md](../../architecture/DATABASE.md), [API.md](../../architecture/API.md), [AUTH.md](../../architecture/AUTH.md), [LGPD.md](../../platform/LGPD.md), [NOTIFICATIONS.md](../../platform/NOTIFICATIONS.md), [GLOSSARY.md](../../dev/GLOSSARY.md), [CRM spec](../growth/crm.md), [Checkout spec](../commerce/checkout.md), [Inbox spec](../team/inbox.md)

---

## 1. Purpose & Scope

The WhatsApp Engine module is the **programmatic messaging backbone** of Ambaril. It owns all WhatsApp communication — transactional (order confirmation, shipping, tracking), marketing (campaigns from CRM segments), cart recovery (triggered by CRM automations), creator notifications (sale/tier/payout alerts), and VIP group management. The integration is **direct to Meta Cloud API** (no BSP intermediary), which eliminates per-message markup fees and gives full control over template submission, webhook handling, and conversation billing.
...
| **Inbox Bridge** | Bidirectional message flow with the Inbox module — outbound messages sent via this engine, inbound messages routed to Inbox tickets |
...
**Out of scope:** This module does NOT own the conversation UI. Customer-facing message threads are rendered and managed by the Inbox module. WhatsApp Engine is the transport layer — Inbox is the interface layer. Email sending is handled separately by Resend integration, not this module.
...
*This module spec is the source of truth for WhatsApp Engine implementation. All development, review, and QA should reference this document. Changes require review from Marcus (admin) or Caio (pm).*
