# ClawdBot Discord Bot (Pulse) — Module Spec

> **Module:** ClawdBot Discord Bot
> **Codename:** Pulse
> **Schema:** `clawdbot`
...
> **References:** [DATABASE.md](../../architecture/DATABASE.md), [API.md](../../architecture/API.md), [AUTH.md](../../architecture/AUTH.md), [NOTIFICATIONS.md](../../platform/NOTIFICATIONS.md), [GLOSSARY.md](../../dev/GLOSSARY.md), [Dashboard spec](../intelligence/dashboard.md)

---

## 1. Purpose & Scope

ClawdBot is the **internal Discord bot and structured reporting engine** of Ambaril. It operates across 9 granular Discord channels, delivering automated data reports on a cron schedule and providing an interactive AI chat interface for ad-hoc data queries. ClawdBot is NOT a general-purpose AI assistant — it is a focused data retrieval and reporting tool that surfaces Ambaril operational, financial, and growth metrics directly inside the team's Discord workspace.
...
| **Real-time alerts** | #alertas channel receives events from the Flare notification system. Stock critical, PCP delays, conversion drops, system errors — all pushed in real-time with severity-coded embeds. |
| **Interactive AI chat** | #geral-ia channel where any team member asks a question in natural language. ClawdBot uses Claude Sonnet to understand the question, generate a read-only SQL query (if data-related), execute it, and format the response. |
| **Bot personality (SOUL.md)** | ClawdBot has a defined personality loaded as system prompt: direto (direct), dados primeiro (data-first), pt-BR informal but professional, no emoji except for trend indicators (arrows). |
...
| **Tavares** | `operations` | Reads production and inventory reports. Receives PCP alerts in #alertas. |
...
**Out of scope:** ClawdBot does NOT modify any data in Ambaril — it is strictly read-only. It does NOT handle customer-facing communication (owned by WhatsApp Engine and Inbox). It does NOT generate dashboards or charts (owned by Dashboard / Beacon). It does NOT replace the admin UI for operational tasks — it surfaces data summaries in Discord for quick team consumption.
...
| US-16 | Tavares | Ask "quais ordens de producao estao atrasadas?" in #geral-ia | I can get a quick list without opening the PCP module | Bot generates SQL against pcp.production_orders + pcp.production_stages. Returns list with order name, expected date, days overdue. |
...
| US-21 | Marcus | Edit prompt templates used by ClawdBot | I can refine the bot's output formatting and personality without code changes | List of templates by category (report/chat/alert). Edit view with template text, variable placeholders preview, and test button. |
...
| bot_response | TEXT | NOT NULL | ClawdBot's full response |
...
```
+-----------------------------------------------------------------------------+
|  ClawdBot > Agendamentos                                    [+ Novo Schedule]|
+-----------------------------------------------------------------------------+
|                                                                               |
|  +------+------------------+------------------+--------+---------+---------+ |
...
|  --- Prompt Template ---                                       |
|  +-----------------------------------------------------------+|
|  | Voce e o ClawdBot, assistente de dados da CIENA.            ||
...
```
+-----------------------------------------------------------------------------+
|  ClawdBot > Logs de Relatorios            [Canal: Todos v] [Status: Todos v]|
...
```
+-----------------------------------------------------------------------------+
|  ClawdBot > Historico de Chat (#geral-ia)      [Buscar: ________________]   |
...
```
+-----------------------------------------------------------------------------+
|  ClawdBot > Templates de Prompt                            [+ Novo Template] |
+-----------------------------------------------------------------------------+
|                                                                               |
|  +--------+---------------------------+----------+--------+                  |
|  | Cat.   | Nome                      | Ativo    | Vars   |                  |
...
| `pcp.stage.overdue` | PCP | Orange (#e67e22) | @Tavares | Production order name, stage name, expected date, days overdue |
...
| No mutations | Database connection uses a read-only PostgreSQL role (`clawdbot_readonly`) with SELECT-only grants on all schemas. |

**SOUL.md Personality (loaded as system prompt):**

```
Voce e o ClawdBot, assistente de dados da CIENA (streetwear brasileira).
...
### 8.1 Inbound (data sources ClawdBot reads FROM)
...
| **PCP** | `pcp.production_orders`, `pcp.production_stages`, `pcp.rework_orders` | Production reports, PCP chat queries |
| **CRM** | `crm.contacts`, `crm.rfm_scores`, `crm.segments`, `crm.cohorts` | Customer analytics queries |
| **Marketing Intelligence** | `marketing.ugc_posts`, `marketing.campaign_metrics`, `marketing.social_metrics` | Marketing reports, campaign performance queries |
...
### 8.2 Outbound (ClawdBot sends TO)
...
|  (all modules)   | event |   ClawdBot     | REST  |  (Haiku/Sonnet)  |
+------------------+       |   (Pulse)      |       +------------------+
                           |                |
                           +-------+--------+
                                   |
...
From [AUTH.md](../../architecture/AUTH.md). ClawdBot has two access contexts: **Discord** (team members interact via Discord channels) and **Admin UI** (authenticated internal users manage bot configuration).
...
**Notes:**
- `admin` (Marcus) has full access to all ClawdBot configuration, logs, and settings. Only admin can modify schedules, templates, and bot settings.
- `pm` (Caio) has read access to schedules, logs, chat history, and templates for monitoring purposes. No write access.
- All other roles have no access to the ClawdBot admin UI. They interact with ClawdBot exclusively through Discord.
- External roles (`b2b_retailer`, `creator`) have zero ClawdBot access.
...
ClawdBot is primarily a **consumer** of Flare events (for #alertas), not an emitter. However, it emits a small set of meta-events for its own monitoring.

### 11.1 Events Consumed (for #alertas dispatch)

ClawdBot subscribes to ALL Flare events configured with `discord` as a delivery channel. See [NOTIFICATIONS.md](../../platform/NOTIFICATIONS.md) for the complete event catalog. Key events:
...
|---|----------|-------|--------|-------|
| OQ-1 | Should ClawdBot support thread replies in Discord, or always post as top-level messages in the channel? | Marcus | Open | Threads could reduce channel noise for reports but add UI complexity. Alerts should always be top-level. |
...
| OQ-4 | What is the monthly Claude API budget for ClawdBot? Need to set alerting thresholds for token usage. | Marcus / Pedro | Open | Estimate: ~$50/month for Haiku reports (9 reports/day * 500 tokens avg * 30 days) + ~$100/month for Sonnet chat (20 queries/day avg * 2000 tokens avg * 30 days). Total ~$150/month. |
...
| OQ-6 | Should ClawdBot support scheduled DM messages to specific team members (e.g., Pedro receives DRE reminder on the 10th)? | Caio | Open | Useful for role-specific reminders but adds complexity to the notification routing. |
...
| OQ-8 | Should ClawdBot log all SQL queries generated by Sonnet for security audit purposes, even if they fail validation? | Marcus | Open | Currently only successful queries are logged in `chat_history.sql_generated`. Rejected mutation queries should also be logged for security monitoring. |

---

*This module spec is the source of truth for ClawdBot implementation. All development, review, and QA should reference this document. Changes require review from Marcus (admin).*
