...
> **References:** [DATABASE.md](../../architecture/DATABASE.md), [API.md](../../architecture/API.md), [AUTH.md](../../architecture/AUTH.md), [NOTIFICATIONS.md](../../platform/NOTIFICATIONS.md), [GLOSSARY.md](../../dev/GLOSSARY.md), [PCP spec](../operations/plm.md), [Marketing Intel spec](../growth/marketing.md)
...
_This module spec is the source of truth for Gestao de Tarefas e Projetos implementation. All development, review, and QA should reference this document. Changes require review from Caio (PM) or Marcus (admin)._
...
Template sets of tasks per operational area. "Lançar Drop" creates 15 tasks automatically (briefing, production, photos, copy, scheduling, live). Standardizes repetitive processes and accelerates onboarding. New table `tarefas.playbooks` with `tasks` JSONB array. Pages: `(admin)/tarefas/playbooks` (CRUD) + "Execute Playbook" modal in Kanban/Gantt.

**User Stories:**

| #       | As a... | I want to...                                                  | Acceptance Criteria                                                                                                                                                       |
| ------- | ------- | ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| US-AX18 | Caio    | Create a playbook with a set of template tasks                | Playbook editor: name, area, list of tasks with title, description, assignee_role, duration_days, dependencies. Save creates reusable template.                           |
| US-AX19 | Caio    | Execute a playbook to create real tasks with calculated dates | "Execute Playbook" button in Kanban. Select playbook + start date. System creates N tasks in `tarefas.tasks` with dates calculated from start + durations + dependencies. |
| US-AX20 | Marcus  | See which playbooks are used most                             | usage_count field incremented on each execution. Sortable column in playbook list.                                                                                        |

Pre-installed playbooks per tenant: "Lançar Drop", "Receber Produção", "Campanha Marketing".

### Meeting Management

Register team meeting notes, link to tasks, and generate AI summaries. New table `tarefas.meetings` with agenda, notes, ai_summary, suggested_tasks. Pages: `(admin)/tarefas/reunioes` (list) + `(admin)/tarefas/reunioes/[id]` (detail).

**User Stories:**

| #       | As a... | I want to...                                                   | Acceptance Criteria                                                                                                                                    |
| ------- | ------- | -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| US-AX21 | Caio    | Create a meeting with agenda topics and add notes during/after | Meeting form: title, date, participants (multi-select users), agenda (text array). Notes field (rich text).                                            |
| US-AX22 | Caio    | Generate an AI summary of the meeting with suggested tasks     | "Gerar Summary" button. ClawdBot (Haiku) summarizes notes into key points + suggests tasks. Suggested tasks shown as cards with "Criar Tarefa" button. |
| US-AX23 | Marcus  | See meeting history searchable by date and participants        | List view with search, date filter, participant filter.                                                                                                |

### Pomodoro Timer

Focus timer integrated in workspace. User selects a task, starts Pomodoro (25min work, 5min break). Client-side only — state persisted in localStorage. No database schema. Widget: floating FAB in bottom-right corner of `(admin)/tarefas`.

**Note:** Low priority. Implement as simple client-side component with no backend persistence.

---

## Calendário Universal com Tags

Central calendar view for the entire company. Replaces the previous 3 separate calendars (Editorial in Tarefas, Content in Marketing, WA Content Calendar in Mensageria).

Each event has **tags**: `marketing`, `produção`, `financeiro`, `comercial`, `criativo`, `suporte`. Modules contribute events automatically:

- PLM: production deadlines, sample deliveries
- Marketing: IG posts, stories, email campaigns, WA broadcasts
- ERP: shipment dates, financial due dates
- CRM: scheduled campaigns, automations
- B2B: wholesale deliveries

Filter by tag, module, or assignee. Monthly/weekly/daily views. Drag-drop to reschedule. Colors by tag (organizational color system from DS v3.2).

**Quem vê o quê:**

- Caio: filtra `marketing` + `criativo` → agenda de conteúdo
- Tavares: filtra `produção` → deadlines de fábrica
- Pedro: filtra `financeiro` → vencimentos
- Marcus: sem filtro → tudo
- Ana Clara: filtra `produção` + `comercial` → envios

---

## Competitive Context

> Fonte: Análise de calls de demo — Audaces ISA (~3h40), Colecao.moda (~25min). Abril 2026.
> Detalhes: [`docs/competitive/audaces-isa.md`](../../competitive/audaces-isa.md), [`docs/competitive/gap-analysis.md`](../../competitive/gap-analysis.md)

### Posicionamento

O módulo Tarefas é o **mais completo** do Ambaril vs concorrentes analisados. **Nenhum dos 3 apps** tem gestão de tarefas integrada:

- Audaces ISA: sem equivalente. Clientes usam Monday + Trello + Jira em paralelo
- Colecao.moda: sem equivalente. Clientes usam ferramentas genéricas
- Inbazz: irrelevante (foco em creators)

### Principal diferencial competitivo

A **integração bidirecional PLM↔Tarefas** (criar OP → auto-cria task para Tavares; stage completo → task auto-completa) é o diferencial mais forte do Ambaril vs Monday/Asana/ClickUp no contexto de marcas de moda DTC. Esta feature não existe em nenhum dos concorrentes analisados.

### Validação de mercado

O Marcus (CIENA) disse na call da ISA que usa Monday + Trello + Jira para gestão. A principal queixa: "são ferramentas genéricas, não específicas de moda." O Ambaril Tarefas resolve isso com:

- Templates de drop lifecycle (Fase 0–5)
- PCP sync automático
- Calendar Editorial integrado
- Vocabulário de moda nativo

### Sem gaps críticos identificados

A análise competitiva não identificou gaps no módulo Tarefas. Todas as funcionalidades dos concorrentes estão cobertas ou superadas. Os únicos pontos menores que surgiram (lembretes manuais, @mention granular) foram incorporados no PLM spec (onde são mais relevantes).
