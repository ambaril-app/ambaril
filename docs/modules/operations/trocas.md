...
> **References:** [DATABASE.md](../../architecture/DATABASE.md), [API.md](../../architecture/API.md), [AUTH.md](../../architecture/AUTH.md), [LGPD.md](../../platform/LGPD.md), [NOTIFICATIONS.md](../../platform/NOTIFICATIONS.md), [GLOSSARY.md](../../dev/GLOSSARY.md), [ERP spec](./erp.md), [Checkout spec](../commerce/checkout.md), [CRM spec](../growth/crm.md), [Inbox spec](../team/inbox.md)
...
**Primary users:**
- **Slimgust (Support):** Creates exchange requests from Inbox conversations, communicates with customers
...
**Out of scope:** This module does NOT handle the original order fulfillment (owned by ERP). It does NOT own the customer conversation (owned by Inbox). It does NOT process payments (Mercado Pago refunds are triggered via ERP financial reconciliation). It does NOT own the store credit redemption UX (owned by Checkout — store credit is applied like a coupon).
...
|---|---------|-------------|-----------|-------------------|
| US-01 | Customer | Request an exchange via WhatsApp/Inbox | I can get the right size/color without visiting a store | Customer sends exchange request via WhatsApp; Slimgust opens exchange form in Inbox; exchange request created with `requested_by = 'support'`; customer receives WhatsApp confirmation with exchange ID |
...
| # | As a... | I want to... | So that... | Acceptance Criteria |
|---|---------|-------------|-----------|-------------------|
| US-06 | Support (Slimgust) | Create an exchange request from within an Inbox conversation | I can process the customer's request without switching contexts | "Abrir troca" button in Inbox conversation detail opens exchange creation form; form pre-fills contact info and recent orders; created exchange is linked to Inbox ticket via `related_exchange_id` |
| US-07 | Support (Slimgust) | View the exchange request timeline alongside the customer conversation | I can see the full context when discussing the exchange with the customer | Inbox right panel shows linked exchange with status timeline, items, and logistics info |
...
| requested_by | trocas.exchange_requested_by | NOT NULL | Who initiated: `customer` (self-service, future) or `support` (via Inbox) |
...
### 4.3 Create Exchange (from Inbox or standalone)
...
|  +-----------------------------------------------------------------+  |
|                                                                       |
|  Vinculado ao ticket: #INB-4521 (Inbox)                              |
|                                                                       |
...
| Exchange history | Inbox -> Trocas | DB query on `trocas.exchange_requests` by `contact_id` | Inbox customer context panel shows recent exchanges |
...
### 7.6 Inbox Module (Exchange from Conversation)

| Interaction | Direction | Mechanism | Description |
|------------|-----------|-----------|-------------|
| Create exchange from ticket | Inbox -> Trocas | Admin API `POST /exchanges` with `inbox_ticket_id` | Inbox "Abrir troca" action creates exchange linked to the ticket |
| Exchange status in ticket | Inbox -> Trocas | DB query on `trocas.exchange_requests` by `inbox_ticket_id` join | Inbox shows linked exchange status in the ticket detail panel |
...
- `support` (Slimgust) can read and create exchanges (via Inbox) but cannot approve, reject, or inspect. She is the intake channel, not the decision-maker.
...
- [ ] **Size swap happy path:** Create exchange from Inbox -> approve -> customer ships (mock tracking) -> receive -> inspect OK -> complete -> verify replacement shipment created and customer notified at each step
...
| OQ-2 | Should we offer self-service exchange requests via "Minha Conta" page, or always require Inbox/support? | Caio / Marcus | Open | Current spec: exchanges are created by support (Slimgust) via Inbox. Self-service would reduce support load but requires a customer-facing UI. Could be a Phase 2 feature. |
