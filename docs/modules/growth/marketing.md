...
_This module spec is the source of truth for Marketing Intelligence implementation. All development, review, and QA should reference this document. Changes require review from Marcus (admin) or Caio (pm)._
...
Editorial calendar for Instagram/TikTok with planned posts per week. Yuri plans content, Sick prepares visuals, Caio approves. Replaces spreadsheet/Monday for content planning. New table `marketing.content_calendar`. Pages: `(admin)/marketing/calendario` (monthly + weekly views).

**User Stories:**

| #       | As a... | I want to...                                                          | Acceptance Criteria                                                                                                                                 |
| ------- | ------- | --------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| US-AX46 | Caio    | See a monthly content calendar with planned posts                     | Calendar view: cards per day. Each card shows: platform icon, content type, title, status badge. Color-coded by platform.                           |
| US-AX47 | Yuri    | Create a content post with platform, type, copy, and visual reference | Form: date, platform (IG Feed/Reels/Stories/TikTok), content_type (product/lifestyle/UGC/BTS), copy, visual reference (URL or DAM asset), hashtags. |
| US-AX48 | Yuri    | Generate post copy with AI using Brand Brain + Persona                | "Gerar Copy" button. AI writes copy based on content type, brand voice (from Brain), target persona, and any calendar events.                       |
| US-AX49 | Caio    | Approve content before it goes to production                          | Status flow: Planned → In Production → Ready → Published. Approve button changes status. Records approved_by.                                       |
| US-AX50 | Caio    | See weekly content gauge: "3/5 posts da semana prontos"               | Summary bar at top of weekly view showing posts ready vs planned for the week.                                                                      |

AI copy generation uses Brand Brain + Personas. Integration with DAM: visual final linked as asset.

### Stories Calendar

Specific planning for Instagram Stories — ephemeral, sequential, interactive format. Implemented as view/filter within Content Calendar with `platform = 'ig_stories'`. Additional fields on `content_calendar`: `story_sequence` (position 1/5), `story_group_id` (groups same-day stories), `interactive_type` (poll/quiz/slider/question/countdown).

### Meta Ads Library Cache

Search and cache competitor ads from Meta Ad Library in a structured way. Evolves the planned "Competitor Watch" with cache + AI insights. New table `marketing.ads_library_cache`. Page: `(admin)/marketing/competitors/ads`.

**User Stories:**

| #       | As a... | I want to...                              | Acceptance Criteria                                                                                             |
| ------- | ------- | ----------------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| US-AX51 | Caio    | Search competitor ads by page name        | Search input + results grid. Ads displayed with image/video thumbnail, copy preview, status (active/inactive).  |
| US-AX52 | Caio    | See AI analysis of competitor ad patterns | AI insight card: dominant themes, formats (video vs image ratio), copy patterns, posting frequency. Cached 24h. |
| US-AX53 | Caio    | Save competitor ads as references         | "Salvar" button bookmarks ad. Saved ads accessible in a "Referências" section.                                  |

Cache TTL: 24h. Meta Ad Library is public — no special auth required. Limit: 10 monitored competitors per tenant.

### Ads Budget Planning

Monthly ad spend planning per platform with ROAS/CPA targets and revenue projections. New table `marketing.ads_budget`. Page: `(admin)/marketing/ads-budget`.

**User Stories:**

| #       | As a... | I want to...                                         | Acceptance Criteria                                                                                             |
| ------- | ------- | ---------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| US-AX54 | Caio    | Plan monthly ad budget per platform with ROAS target | Form: month, platform (Meta/Google/TikTok), budget, target ROAS, target CPA. Projected revenue = budget × ROAS. |
| US-AX55 | Caio    | See budget execution dashboard card                  | Card on Marketing Intel: "Meta Ads: R$8.5k / R$10k budget, ROAS 3.8x (meta: 4x)". Progress bar for spend.       |

One entry per month/platform (UNIQUE). Actual spend manual in v1, API integration in v2. Feeds DRE as "Despesas de Marketing".

### Social Consulting Metrics

Instagram metrics dashboard (followers, reach, profile visits, bio clicks) with trends and AI insights. New table `marketing.social_metrics`. Page: `(admin)/marketing/social`.

**User Stories:**

| #       | As a... | I want to...                                | Acceptance Criteria                                                                                               |
| ------- | ------- | ------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| US-AX56 | Caio    | See Instagram metrics cards with deltas     | Cards: Followers (with +/-), Reach (week), Profile Visits, Bio Clicks. Delta vs previous period with color arrow. |
| US-AX57 | Caio    | See 30/90 day trend charts                  | Line charts per metric. Toggle 30d/90d. Hover shows date + value.                                                 |
| US-AX58 | Caio    | Get weekly AI insight on social performance | AI card: "Reels had 3x more reach than static posts this week". Generated weekly by ClawdBot (Haiku).             |

v1: manual data entry (Caio inputs from IG Insights). v2: Instagram Graph API integration via `social` provider.

### Inside Commerce Audit

Structured checklist for periodic e-commerce site audit (UX, offer, copy, SEO, speed). Monthly review with score. New table `marketing.commerce_audits`. Page: `(admin)/marketing/auditoria`.

**User Stories:**

| #       | As a... | I want to...                                         | Acceptance Criteria                                                                                                  |
| ------- | ------- | ---------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| US-AX59 | Caio    | Run a monthly site audit with structured checklist   | Checklist grouped by category (Homepage, PDP, Cart, Checkout, Mobile, SEO). Each item: Yes/No/Partial + notes field. |
| US-AX60 | Caio    | See overall audit score and comparison with previous | Score = % of items "Yes" or "Partial". Side-by-side with previous audit: green (improved), red (worsened).           |
| US-AX61 | Caio    | Get AI recommendations based on failed items         | AI card with actionable suggestions for items marked "No". Generated by ClawdBot (Haiku).                            |

Pre-defined checklist (~30-50 items) installed per tenant. Admin can customize items. SEO/AEO items included in the checklist.

---

## Consolidated Features (formerly E6, N2)

> **Nota sobre calendário:** Eventos de conteúdo de marketing são criados neste módulo mas aparecem no Calendário Universal (módulo Tarefas) com tag `marketing`. Marketing é inteligência + análise; Tarefas é scheduling + execução.

### Prova Social / UGC no Site

Curated UGC gallery (not automated) with editorial aesthetic — Yuri/Sick select best UGCs, apply brand treatment. Appears as community lookbook, NOT product reviews. "A galera CIENA." Shoppable: click photo → see product → checkout. Technically: Marketing collects UGC → team curates → DAM → Shopify embed with custom design.

### SEO & AEO (Search + AI Visibility)

- Product Data Enrichment Score ("your data is 67% ready for AI shoppers")
- Schema.org / Structured Data Generator (automatic JSON-LD for all products)
- AI-Readiness Score (products ready for agentic commerce)
- Content Suggestions (keywords + gaps identified vs. competitors)
- Technical SEO Audit (meta tags, canonical, sitemap, robots.txt)
- AEO Optimization (structured FAQs, granular attributes for AI browsers)
