...
**Out of scope:** This module does NOT handle message delivery directly. WhatsApp dispatch is delegated to the Mensageria module; email dispatch is delegated to Resend via the `packages/integrations/resend/` client. CRM orchestrates; channels deliver.
...
_This module spec is the source of truth for CRM implementation. All development, review, and QA should reference this document. Changes require review from Marcus (admin) or Caio (pm)._
...
Create satisfaction surveys (NPS + custom questions) with public link for customers to respond. Response dashboard with AI insights. New tables: `crm.surveys`, `crm.survey_questions`, `crm.survey_responses`, `crm.survey_answers`. Pages: `(admin)/crm/pesquisas` (CRUD + dashboard), `/pesquisa/[slug]` (public response page, no auth, mobile-first).

**User Stories:**

| #       | As a...  | I want to...                                         | Acceptance Criteria                                                                                                                          |
| ------- | -------- | ---------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| US-AX24 | Caio     | Create a survey with NPS + custom questions          | Survey builder: title, slug, brand color/logo, questions (NPS 0-10, open text, single/multi choice, scale). Preview before publishing.       |
| US-AX25 | Customer | Respond to a survey via public link on mobile        | Public page at `/pesquisa/[slug]`. Mobile-first, branded with tenant colors. No login required. Rate limit: 1 response per email per survey. |
| US-AX26 | Caio     | See NPS score, distribution, and responses dashboard | Dashboard: NPS score gauge, promoter/neutral/detractor distribution (donut), responses table (sortable), AI insights section.                |
| US-AX27 | Caio     | Send survey link via WhatsApp Engine                 | Integration with WhatsApp campaigns: "Enviar Pesquisa" action sends survey link to selected segment.                                         |

NPS calculation: % promoters (9-10) minus % detractors (0-6). LGPD: respondent email is personal data — respect right to deletion.

### Buyer Personas (AI-Generated)

Generate buyer personas from real CRM data (RFM segments, purchase history, exchanges, location). Data-driven, not text-based like competitors. New table `crm.buyer_personas`. Pages: `(admin)/crm/personas` (gallery) + `(admin)/crm/personas/[id]` (detail).

**User Stories:**

| #       | As a... | I want to...                                             | Acceptance Criteria                                                                                                                                                                                                                   |
| ------- | ------- | -------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| US-AX28 | Caio    | Generate buyer personas from actual customer data        | "Gerar Personas" button. AI (Sonnet) analyzes: top RFM segments, most sold categories, ticket ranges, exchange reasons, geographic distribution. Generates 3-5 personas with name, demographics, behaviors, pain points, motivations. |
| US-AX29 | Yuri    | View personas as visual cards with avatar and key traits | Persona gallery: card per persona with avatar, name, age range, location, key pain points, buying behavior summary. Click for full detail.                                                                                            |
| US-AX30 | Caio    | Edit personas manually to add qualitative insights       | Editable fields on persona detail page. Source field tracks 'ai' vs 'manual' edits.                                                                                                                                                   |

Max 5 active personas per tenant. Personas serve as input for: ClawdBot context, Creative Script Gen, Content Calendar AI.

### CRM por Cluster

Automatic customer segmentation into behavioral clusters with pre-defined actions per cluster. Evolves existing RFM from "numeric score" to "named clusters with action playbooks". New tables: `crm.customer_clusters`, `crm.cluster_actions`.

**Clusters:**

- **First-timers** — 1 purchase, last 30 days
- **Returning** — 2+ purchases
- **VIP** — 5+ purchases OR total > R$2,000
- **At-Risk** — purchased in last 90 days but frequency declining
- **Churned** — last purchase > 90 days
- **Whales** — top 5% by lifetime value

**User Stories:**

| #       | As a... | I want to...                                                   | Acceptance Criteria                                                                                                               |
| ------- | ------- | -------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| US-AX31 | Caio    | See customers automatically segmented into behavioral clusters | CRM → Segmentos → "Clusters" tab. Each cluster shows: name, count, total revenue, avg ticket. Donut chart distribution.           |
| US-AX32 | Caio    | Configure actions per cluster (email, WhatsApp, manual)        | Cluster detail: list of actions with channel, trigger timing, template. Toggle active/inactive.                                   |
| US-AX33 | Caio    | See cluster migration over time                                | Monthly sankey/flow diagram: how many customers moved between clusters (e.g., VIP → At-Risk). Flare alert on negative migrations. |

Clusters recalculated daily via Vercel Cron (02:00 BRT).

---

## Consolidated Features (formerly E1-CRM, E11-CRM, N3)

### Waitlist + VIP Whitelist (CRM side)

VIP whitelist management for drops. VIP segment creation and rules. Waitlist-to-contact conversion (CPF/email matching). Integration with Checkout VIP gate. CRM stores VIP status on contacts.

### Reviews & Ratings (CRM side)

> CRM owns the review request automation flow and stores review data on contact profiles. The public display of reviews (star snippets, review teaser widgets) belongs to the **Onsite module** (`onsite.md`).

#### Review Request Automation Flow

Triggered automatically after `order.delivered` event (or ERP delivery confirmation). Multi-step, multi-channel:

| Step | Delay from delivery     | Action                                  | Channel                   |
| ---- | ----------------------- | --------------------------------------- | ------------------------- |
| 1    | +7 dias                 | Solicitar avaliação da loja             | Email + SMS (opcional WA) |
| 2    | +10 dias                | Solicitar avaliação dos produtos        | Email + SMS (opcional WA) |
| 3    | +13 dias                | Lembrete (se nenhuma avaliação enviada) | Email                     |
| 4    | Imediato após avaliação | Agradecimento                           | Email                     |
| 5    | Quando admin responde   | Notificar cliente da resposta           | Email                     |

- Each step skips if contact has already submitted the requested review type
- SMS uses template variable `*|SHOP_REVIEW_URL|*` and `*|PRODUCT_REVIEW_URL|*`
- WhatsApp step is optional (requires consent check)
- Stop condition: if contact submits both store + product review, remaining steps cancel

**Review URL mechanics:** Each request generates a signed, time-limited review URL (`/avaliar/[token]`). Token encodes: `contact_id`, `order_id`, `review_type`, `exp`. No login required. Mobile-first form.

#### Review Collection Form

Public form at `/avaliar/[token]`:

- Star rating 1-5 (required)
- Review title (optional, max 80 chars)
- Review body (optional, max 500 chars)
- Photo upload (max 3 photos, via R2)
- Product-specific form shows product image and name
- LGPD: reviewers consent is implicit in the signed token (linked to their purchase). Photos stored under review UUID in R2.

#### Review List & Management

Admin page at `(admin)/crm/avaliacoes`:

| Column         | Detail                                   |
| -------------- | ---------------------------------------- |
| Contact        | Name + CPF masked + link to CRM profile  |
| Produto / Loja | Review type                              |
| Nota           | Star rating (1-5)                        |
| Texto          | Review body (truncated, expand on click) |
| Fotos          | Thumbnail count, click to view           |
| Status         | `pendente` / `publicado` / `ocultado`    |
| Data           | Submission date                          |
| Resposta       | "Responder" CTA or response preview      |

- Filter by rating (1-5 stars), type (store/product), status, date range
- Bulk publish / hide
- Admin reply: free text response, sends notification email to customer
- Low-rating alert: review ≤ 2 stars creates Flare event → Discord #alertas + support team notification

#### Review Stats Dashboard

Dashboard card on CRM overview:

- Average rating (all time + vs last 30 days)
- Total reviews (store + product, with breakdown)
- Star distribution (1-5, as horizontal bar chart)
- Email sent count + open rate for review requests
- Reviews received / requests sent ratio (response rate %)

#### Review Data on Contact Profile

Each `crm.contacts` profile shows:

- Review count (store reviews + product reviews)
- Average rating given
- Last review date
- Reviews tab: list of their submitted reviews with dates and ratings
- Low-rater flag: contacts who consistently give ≤ 2 stars flagged for support awareness

#### Backfill (Import Historical Reviews)

For tenants migrating from other platforms (Shopify Product Reviews, Fera.ai, etc.):

- CSV import: columns `order_id`, `email`, `rating`, `title`, `body`, `date`
- Matching: CPF/email → links to existing `crm.contacts`
- Estimates import count before executing (preview: "~340 reviews from last 30 days")
- Backfilled reviews are marked `source='import'` (distinct from collected reviews)
...
CREATE TABLE crm.reviews (
  ...
  product_id   UUID REFERENCES erp.products(id), -- null for store reviews
  rating       SMALLINT NOT NULL CHECK (rating BETWEEN 1 AND 5),
  title        TEXT,
  body         TEXT,
  photo_urls   TEXT[],
  status       TEXT NOT NULL DEFAULT 'pendente' CHECK (status IN ('pendente','publicado','ocultado')),
  admin_reply  TEXT,
  replied_at   TIMESTAMPTZ,
  source       TEXT DEFAULT 'collected', -- 'collected' | 'import'
  token_hash   TEXT UNIQUE, -- signed request token hash
...
Review request automation uses existing `crm.automations` + `crm.automation_runs` tables with `trigger_type='order.delivered'` and `template_id` pointing to review request templates in Mensageria.

### On-Site Tools (O1-O9)

> **Architecture note:** On-Site widgets are managed via the standalone **Onsite module** (`onsite.md`). The Onsite module handles all widget creation, targeting, injection, and analytics. The CRM-data-driven widgets below (O1-O9) are specifically CRM-owned features that use CRM segment data as targeting rules and feed data back into CRM. They are configured in the Onsite module but listed here as CRM capabilities.

Provider-agnostic widget injection: a single `<script>` tag installed on any ecommerce platform (Shopify, Nuvemshop, Vtex, WooCommerce). Optional platform-specific app extensions available for deeper context (cart state, checkout events). CRM segments power the targeting rules.

- O1: Smart Popup Builder (targeting by CRM segment, behavior, device — lead capture feeds into `crm.contacts`)
- O2: Social Proof Notifications (real purchase data from ERP: "Maria just bought X" — uses `erp.orders`)
- O3: Product Recommendation Widget (collaborative filtering from CRM purchase data)
- O4: Size Quiz (zero-party data → CRM contact profile update → size recommendation)
- O5: Drop Countdown (timer + opt-in WA/email notification → CRM automation trigger)
- O6: Creator Showcase (curated UGC shoppable gallery — uses `creators` module data)
- O7: Back-in-Stock Alert (waitlist → CRM automation triggers when ERP stock > 0)
- O8: Sticky Cart (persistent cart across all pages — ERP-integrated)
- O9: Live Shopping Widget (future — streaming + real-time purchase)

---

## Live Events / Visitor Deanonymization (Ambaril Tracker)

> **Ambaril Tracker** is the behavioral data infrastructure that powers the CRM. It runs in two phases: MVP (Phase 1A) establishes passive identification; v2 (Phase 1B) adds fingerprinting and active identity injection. The Live Events dashboard surfaces this data in real time and maps customer journeys.

### Purpose

The goal is not a real-time "activity feed" (though that's a byproduct). The primary goal is:

1. **Deanonymization** — Match anonymous website visitors to existing CRM contacts, OR capture new visitors into the CRM before they purchase
2. **Journey mapping** — Reconstruct the full path a contact took before converting (product views → cart → checkout → purchase)
3. **Behavioral enrichment** — Enrich `crm.contacts` with behavioral signals (product interests, frequency of visits, last active date, dropout point in funnel)
4. **Retargeting fuel** — Feed Meta CAPI and custom audiences from behavioral events, not just purchases

### Ambaril Tracker MVP (Phase 1A)

Installed as a Liquid snippet in Shopify theme (`{{ 'ambaril-tracker.js' | asset_url | script_tag }}`). Works on any platform via manual `<script>` install.

**Identification methods (passive):**
| Method | How | Confidence |
|--------|-----|-----------|
| Liquid `{{ customer }}` | Shopify injects logged-in customer data (email, name, id) | High — verified identity |
| `_fbp` / `_ga` cookie | Match Facebook Browser ID or GA Client ID to known contact | Medium — probabilistic |
| `_amb_vid` cookie | Ambaril's own persistent visitor ID (UUID, 365-day cookie) | Medium — device-bound |
| Email form submit | Onsite popup, newsletter signup — resolves to contact on submit | High — explicit consent |
| Checkout UTM | UTM params at checkout → contact attribution | High — purchase-linked |

**Events captured:**

- `page_view` — URL, title, referrer, UTM params
- `product_view` — product ID, variant, price, source
- `add_to_cart` — product ID, quantity, cart value
- `remove_from_cart` — product ID
- `checkout_start` — cart value, item count
- `checkout_step` — step name (address/payment/review)
- `purchase` — order ID (merges with ERP data)
- `session_start` / `session_end` — session duration, page count

All events sent via `POST /api/v1/tracker/event` with `visitor_id` (from `_amb_vid` cookie) and optional `contact_id` (if identified).

### Ambaril Tracker v2 (Phase 1B)

**Identity injection in links:** CRM campaigns (WA/email/SMS) embed a `?_amb_cid={contact_id}` param. On click, Tracker resolves and stores `contact_id` → `visitor_id` mapping. Future page views from this device are now identified.

**ThumbmarkJS fingerprinting:** Device fingerprint (canvas, audio, WebGL, fonts, screen) creates a `fingerprint_hash`. On first visit from a known contact's device, fingerprint → `visitor_id` linkage is stored. Increases identification confidence for returning visitors who cleared cookies.

**Multi-layer persistence:** `_amb_vid` stored in: cookie (1st party), localStorage, sessionStorage, IndexedDB. Any surviving layer restores identity after partial cookie deletion.

**Retroactive attribution:** When an anonymous visitor converts, their entire pre-purchase event history (via `visitor_id`) is retroactively attributed to the newly created/identified contact.

### Live Events Dashboard

Page: `(admin)/crm/live-events`

**Metrics strip (7-day rolling):**
| Metric | Source |
|--------|--------|
| Visitantes únicos | `tracker.sessions.distinct(visitor_id)` |
| Identificados | `tracker.sessions WHERE contact_id IS NOT NULL` |
| Visualizações de produto | `tracker.events WHERE type='product_view'` |
| Adicionado ao carrinho | `tracker.events WHERE type='add_to_cart'` |
| Pedidos realizados | `erp.orders` |
| Novos contatos via tracker | `crm.contacts WHERE acquisition_source='tracker'` |

**Real-time activity feed (last 50 events, polling 10s):**

- Identified contacts show name + CRM segment + link to profile
- Unidentified visitors show `Visitante #[short_id]` with device/location
- Each row: event type icon + product image (if product_view) + time ago
- Click on identified contact → CRM profile in slide-over

**Journey map (per contact):**
On contact profile → "Jornada" tab:

- Timeline of all events: pages visited, products viewed, cart actions, messages opened, purchases
- Time between each event
- Attribution: which campaign touchpoint preceded the purchase
- "Funil" summary card: `Visitou → Adicionou ao carrinho → Comprou` with conversion rates per stage

**Funnel analytics:**
Macro funnel view at `(admin)/crm/live-events/funil`:

- Total visitors → product viewers → cart adds → checkout starts → purchases
- Conversion % at each step, delta vs previous period
- Dropout analysis: where in the funnel do most unidentified visitors drop?

### Database additions

```sql
-- Tracker schemas
CREATE TABLE tracker.visitors (
  id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id       UUID NOT NULL REFERENCES global.tenants(id),
  visitor_id      TEXT NOT NULL UNIQUE, -- '_amb_vid' value
  contact_id      UUID REFERENCES crm.contacts(id), -- null if unidentified
  fingerprint_hash TEXT,
  identification_method TEXT, -- 'liquid_customer' | 'utm_link' | 'form_submit' | 'fingerprint'
  first_seen_at   TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  last_seen_at    TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at      TIMESTAMPTZ NOT NULL DEFAULT NOW()
...
CREATE TABLE tracker.events (
  ...
  event_type   TEXT NOT NULL, -- 'page_view' | 'product_view' | 'add_to_cart' | etc.
...
CREATE INDEX ON tracker.events (tenant_id, visitor_id, created_at DESC);
CREATE INDEX ON tracker.events (tenant_id, contact_id, created_at DESC);
CREATE INDEX ON tracker.events (tenant_id, event_type, created_at DESC);
```

### Future: Session Recording (v3, roadmap)

> Out of scope for v1-v2. Planned when Ambaril reaches scale.

- Full page session recording (mouse movements, clicks, scroll, form interactions)
- Privacy-safe: mask PII fields (CPF, email, card numbers) before recording
- Linked to `tracker.visitors` — watch the session of any contact
- Rage click detection, dead click analysis
- Heatmaps per page (aggregated click density)
- Requires separate storage infrastructure (R2 with lifecycle rules for auto-deletion after 30 days)
- LGPD: explicit consent required; session recording opt-out toggle in privacy settings

---

## Visual Flow Builder

> **Automation Editor** (referenced in Phase 1B) is implemented as a drag-and-drop visual flow builder. Goal: ManyChat-level usability — non-technical users (Caio, Slimgust) can build complex multi-step automation flows without writing code.

### Concept

The 23 pre-built automations (T1-T3, CR1-CR4, L1-L6, R1-R5, etc.) are fixed sequences. The Visual Flow Builder enables **custom automations** — arbitrary flows with branching logic, channel mixing, and conditional paths. Think of it as "build your own automation."

### Canvas Layout

Page: `(admin)/crm/automacoes/nova` (blank canvas) or `(admin)/crm/automacoes/[id]/editor`

- **Left panel:** Node palette with drag-and-drop items
  - **Trigger nodes:** Evento (order.paid, cart.abandoned, etc.), Segmento (contact enters segment), Agendado (cron schedule), Webhook
  - **Action nodes:** Enviar WhatsApp, Enviar Email, Enviar SMS, Aguardar (delay), Atualizar contato (set tag/field), Adicionar a segmento, Remover de segmento, Notificar equipe (Flare), Encerrar fluxo
  - **Logic nodes:** Condição (if/else branch), Divisão A/B (split traffic %), Aguardar evento (wait until condition met within timeout)
- **Main canvas:** Infinite scroll, zoom in/out. Nodes connected by arrows. Drag to reposition. Auto-layout button.
- **Right panel (context):** Click any node → configure its properties (message template, delay duration, condition rules, etc.)

### Node Detail — Key Nodes

**Enviar WhatsApp node:**

- Select pre-approved Meta template (dropdown with search)
- Map template variables to contact fields (`{{1}}` → `contact.name`, `{{2}}` → `order.number`, etc.)
- Preview rendered message
- Error path: if delivery fails → optional branch

**Condição (If/Else) node:**

- Condition builder: contact field + operator + value
  - Examples: `rfm_segment = 'champion'`, `total_orders > 3`, `tags contains 'vip'`, `last_purchase_at < 30 days ago`
- Boolean: AND / OR groups
- Two output paths: TRUE branch, FALSE branch
- Optional: ELSE (no match) path

**Divisão A/B node:**

- % split between 2-4 paths (must sum to 100%)
- Each path runs independently
- Winner selection: after N days, auto-merge to winning path by conversion rate

**Aguardar (delay) node:**

- Duration: minutes / hours / days / weeks
- Smart wait option: "Até horário comercial" (next business day 10:00 BRT)
- Calendar wait: "Até dia específico" (e.g., birthday)

**Aguardar evento (wait-until) node:**

- Wait for: `order.placed`, `message.replied`, `link.clicked`, `form.submitted`
- Timeout: if event not received within N hours/days → exit path or alternative branch
- Use case: "Wait for cart recovery click for 2h, then send follow-up if not clicked"

### Analytics Overlay

On saved flow, toggle "Analytics view":

- Each node shows: contacts entered, contacts exited successfully, dropout count
- Each connection arrow shows: message count, delivered rate, click rate (if message node)
- Sankey visualization of contact flow through the full automation
- Time-to-complete histogram: how long contacts take to traverse the flow

### UX Rules (ManyChat-level)

1. **Immediate feedback:** Drag a node → it snaps to grid. Connect two nodes → arrow draws instantly
2. **Inline edit:** Double-click any node to edit without opening side panel
3. **Template preview:** Message nodes show rendered preview thumbnail on the canvas
4. **Error detection:** Invalid connections (e.g., two trigger nodes) highlighted in red with tooltip explanation
5. **Undo/redo:** Full history (up to 50 actions)
6. **Test mode:** "Executar teste" — runs the flow against a selected test contact in simulation, logs each step result, never sends real messages
7. **Version history:** Each "Publicar" creates a named version. Roll back to any previous version.
8. **One-click templates:** Pre-built flow templates in the library (welcome series, cart recovery, reactivation, birthday). Import into canvas and customize.

### Business Rules

| #    | Rule                                                                                                                          |
| ---- | ----------------------------------------------------------------------------------------------------------------------------- |
| VF-1 | Only one trigger node per flow                                                                                                |
| VF-2 | Every path must end in "Encerrar fluxo" or loop back (cycle detection: loops must have delay nodes to prevent infinite loops) |
| VF-3 | Published flows execute immediately for contacts who enter the trigger condition                                              |
| VF-4 | Editing a published flow creates a draft — changes only apply to new contacts entering after re-publish                       |
| VF-5 | Contact can only be in one instance of a given flow at a time (prevents spam from re-triggered automation)                    |
| VF-6 | LGPD consent is checked at each message node, not at flow entry                                                               |
| VF-7 | Max 50 nodes per flow (performance guard)                                                                                     |
