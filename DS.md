...
### 2.4 Accent — Máximo contraste (sem cor)

Filosofia: **zero accent colorido**. O botão primário é o elemento de maior contraste da tela — preto sobre branco em light, branco sobre escuro em dark. A identidade vem do contraste, não da cor.

| Token | Light | Dark | Uso |
|---|---|---|---|
| `--btn-primary-bg` | `#0F172A` | `#F7F8FA` | Fill do Button primary |
| `--btn-primary-text` | `#FFFFFF` | `#0C0E13` | Texto sobre btn-primary |
| `--btn-primary-hover` | `#1E293B` | `#FFFFFF` | Hover state do Button primary |

**Regras de uso:**
- Botão primário = elemento mais escuro em light / mais claro em dark. Máximo contraste local.
- Sidebar item ativo: `border-left: 2px solid var(--border-strong)` + background shift
- **Nunca** usar cor para destacar ações — usar contraste
- A ausência de cor enquanto todos usam cor é uma posição, não timidez
...
- **Item ativo:** border-left 2px `--border-strong` + background 3% branco
