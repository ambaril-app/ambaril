...
### 2.4 Accent — Âmbar (uso cirúrgico)

Philosophia: **um único accent colorido**, usado só onde existe uma ação para tomar. Não é decoração. Não aparece em charts, badges, bordas ou backgrounds de card.

| Token | Valor | Uso |
|---|---|---|
| `--accent` | `oklch(65% 0.12 60)` | Fill do Button primary + active state de navegação |
| `--accent-hover` | `oklch(60% 0.13 60)` | Hover state do Button primary |
| `--accent-muted` | `oklch(65% 0.12 60 / 0.10)` | Background de highlight pontual (ex: onboarding step ativo) |
| `--accent-text` | `#FFFFFF` | Texto sobre --accent |

**Regras de uso:**
- `--btn-primary-bg` = `--accent` (substituiu `#0F172A` light / `#16181F` dark)
- Sidebar item ativo: `border-left: 2px solid var(--accent)` (substituiu `--text-tertiary`)
- **Nunca** em: charts, tabelas, badges status, borders de card, backgrounds de seção, gradient text
- O âmbar é raro por design. Toda instância extra dilui o impacto.
...
|---|---|---|
| **Bricolage Grotesque** | Display — identidade de marca | H1 de módulo, KPI heroes, display numbers, empty state titles |
| **DM Sans** | Interface — tudo o resto | H2, H3, body, labels, botões, inputs, tooltips, metadados |
| **DM Mono** | Dados — valores numéricos | R$, quantidades, IDs, datas, códigos |

**Regra de scope do Bricolage (Opção A — Presença cirúrgica):**
Bricolage aparece em momentos de identidade — onde o impacto visual justifica a distinção. DM Sans assume 95% da interface. A raridade é intencional: frequência menor = impacto maior.

**Onde Bricolage APARECE:**
- H1 de cada módulo/página (ex: "Creators", "Pedidos", "Financeiro")
- KPI heroes em dashboards (o número grande: "R$ 45.000")
- Títulos de empty states com impacto
- Seções de onboarding de alto nível

**Onde Bricolage NÃO aparece:**
- H2, H3 dentro de cards ou seções
- Labels, botões, inputs, badges
- Tabelas e dados operacionais
- Sidebar, topbar, menus

**Importar:**
```html
<link href="https://fonts.googleapis.com/css2?family=Bricolage+Grotesque:opsz,wght@12..96,400;12..96,500;12..96,600&family=DM+Sans:opsz,wght@9..40,300;9..40,400;9..40,500;9..40,600;9..40,700&family=DM+Mono:wght@400&display=swap" rel="stylesheet">
```
...
|---|---|---|---|---|---|---|
| Display | 48px | Bricolage Grotesque | 500 | -0.02em | 1.1 | KPI heroes, números grandes |
...
| Small | 12px | DM Sans | 400 | +0.01em | 1.5 | Timestamps, metadados, captions |
| Label | 11px | DM Sans | 500 | +0.04em | 1.0 | Labels de seção (uppercase) |
...
| Sidebar ativo | border-left `--accent` + 3% branco bg | border-left `--accent` + bg `--bg-base` (branco) |
...
|---|---|---|---|
| Primário | bg `--accent`, text `--accent-text`, shadow-sm | bg `--accent`, text `--accent-text`, shadow-sm | Ação principal (1 por área) |
...
- **Item ativo:** border-left 2px --accent + background 3% branco
...
:root {
  ...
  /* Accent (âmbar — uso cirúrgico: Button primary + nav active state) */
  --accent: oklch(65% 0.12 60);
  --accent-hover: oklch(60% 0.13 60);
  --accent-muted: oklch(65% 0.12 60 / 0.10);
  --accent-text: #FFFFFF;
...
.dark {
  ...
  /* Buttons (accent compartilhado com :root — sem override aqui) */
