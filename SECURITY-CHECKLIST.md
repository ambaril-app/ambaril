...
- Checks marcados `[P1]` são obrigatórios em toda Wave; `[P2]` become mandatory when a module has completed 3 or more waves (i.e., has sufficient code surface for these checks to be meaningful) — before wave 3, P2 checks are recommended but not blocking; `[P3]` antes do deploy
...
| S04 | Variáveis sensíveis NÃO usam prefixo `NEXT_PUBLIC_`          | P1         | NEXT*PUBLIC* expõe no client bundle |
...
| **P2**     | 22 checks     | Mandatory when module has completed 3 or more waves (sufficient code surface). Before wave 3, recommended but not blocking. Obrigatório antes de deploy staging |
